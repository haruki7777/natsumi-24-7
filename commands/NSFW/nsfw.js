...
import { nsfwCategories } from "../../utils/imageFetchers.js";
  ...
  async execute(interaction) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        .setCustomId(`nsfw_category_${interaction.user.id}`)
        .setPlaceholder("NSFW 카테고리 선택")
        .addOptions(nsfwCategories.map((category) => ({
          label: category.name,
