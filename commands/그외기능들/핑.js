...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const client_ping = client.ws.ping,
      command_ping = msg.createdTimestamp - interaction.createdTimestamp;
    let ping_st, command_st;
    // "🟠,🔴,🟢"
    if (client_ping < 250) {
      ping_st = "🟢";
    } else if (client_ping < 500) {
      ping_st = "🟠";
    } else if (client_ping < 1000) {
      ping_st = "🔴";
    }

    if (command_ping < 500) {
      command_st = "🟢";
    } else if (command_ping < 1000) {
      command_st = "🟠";
    } else if (command_ping < 2000) {
      command_st = "🔴";
    }
    let embed_color = {
      "🟢": "#00FF00",
